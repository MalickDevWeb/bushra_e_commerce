...
async function main() {
  ...
  for (const user of users) {
    const orderCount = faker.number.int({ min: 0, max: 7 }); // Some VIPs, some new
    for (let i = 0; i < orderCount; i++) {
      ...
      await prisma.order.create({
        data: {
          ...
          totalAmount: product.price * qty + 2000, // + delivery
          status: "DELIVERED",
          paymentMethod: "WAVE",
          paymentStatus: "PAID",
          createdAt: pastDate,
  ...
  if (!aissatou) {
    ...
    // Give her 4 orders of Oud in the past 90 days, but last one was 67 days ago
    ...
    for (let i = 0; i < 4; i++) {
      await prisma.order.create({
        data: {
          ...
          status: "DELIVERED",
          createdAt: new Date(Date.now() - (67 + i * 31) * 24 * 60 * 60 * 1000), // 67 days ago, then 98, etc.
          items: {
  ...
  // 5. Setup Segments and Campaigns
