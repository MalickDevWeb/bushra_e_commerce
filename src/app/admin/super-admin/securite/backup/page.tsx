  ...
  return (
    <div className="flex flex-col gap-6 w-full text-[#e8e1d3] max-w-[1600px] mx-auto pb-10">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold font-serif text-[#d4af37]">Sauvegardes Système (Backups)</h1>
          <p className="text-[13px] text-[#a89b82] mt-1">Gérez les archives et points de restauration de la plateforme</p>
        </div>
        <button className="flex items-center gap-2 px-4 py-2 bg-[#d4af37] text-[#0a0a0a] rounded-md font-semibold text-sm hover:bg-[#c59b32] transition-colors shadow-[0_0_15px_rgba(212,175,55,0.2)]">
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-[#d4af37]/20 bg-[#1c1813]">
              <th className="py-4 px-5 text-[11px] font-bold uppercase tracking-wider text-[#a89b82]">ID Sauvegarde</th>
              <th className="py-4 px-5 text-[11px] font-bold uppercase tracking-wider text-[#a89b82]">Date de création</th>
              <th className="py-4 px-5 text-[11px] font-bold uppercase tracking-wider text-[#a89b82]">Type</th>
              <th className="py-4 px-5 text-[11px] font-bold uppercase tracking-wider text-[#a89b82]">Taille</th>
          ...
          <tbody className="divide-y divide-[#d4af37]/10">
            {backups.map((bk, i) => (
              <tr key={i} className="hover:bg-[#d4af37]/5 transition-colors group">
                <td className="py-4 px-5 text-[13px] font-mono text-[#d4af37]">{bk.id}</td>
                <td className="py-4 px-5 text-[13px] text-[#e8e1d3]">{bk.date}</td>
                <td className="py-4 px-5 text-[12px] text-[#a89b82]">{bk.type}</td>
                <td className="py-4 px-5 text-[13px] text-[#e8e1d3] font-mono">{bk.size}</td>
                <td className="py-4 px-5">
                  <span className={`px-2.5 py-1 text-[10px] rounded-full border ${bk.status === "Succès" ? "bg-[#2ecc71]/15 text-[#2ecc71] border-[#2ecc71]/30" : "bg-[#e74c3c]/15 text-[#e74c3c] border-[#e74c3c]/30"}`}>
                    {bk.status}
                  </span>
                </td>
                <td className="py-4 px-5 text-right flex items-center justify-end gap-2">
                  <button className="text-[#a89b82] hover:text-[#d4af37] text-xs px-2 py-1" title="Télécharger">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="w-4 h-4"><path strokeLinecap="round" strokeLinejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5M16.5 12L12 16.5m0 0L7.5 12m4.5 4.5V3" /></svg>
                  </button>
                  <button className="text-[#a89b82] hover:text-red-400 text-xs px-2 py-1" title="Supprimer">
