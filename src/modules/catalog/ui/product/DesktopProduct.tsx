...
export function DesktopProduct({ product }: { product: CatalogProduct | null }) {
  ...
  const gallery = [product.image, product.image2, product.image3].filter(
    ...
    <div className="hidden lg:flex w-full min-h-screen bg-[#090705]">
      <main className="flex-1 max-w-[1600px] mx-auto px-10 py-12">
        ...
        <nav className="flex text-sm text-[#a89b82] mb-10">
          ...
          <span className="mx-2">/</span>
          <Link href={`${ROUTES.shop}?category=${product.category.slug}`} className="hover:text-[#d4af37] transition-colors capitalize">{product.category.name}</Link>
          <span className="mx-2">/</span>
