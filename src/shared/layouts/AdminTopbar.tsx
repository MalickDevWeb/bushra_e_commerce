...
import { getAdminHeaderData } from "@/modules/admin/actions/header.actions";
...
export function AdminTopbar({ onToggleSidebar }: AdminTopbarProps) {
  const [headerData, setHeaderData] = useState<Awaited<ReturnType<typeof getAdminHeaderData>>>(null);

  useEffect(() => {
    void getAdminHeaderData().then(setHeaderData);
  }, []);

  const roleLabel = headerData?.role === "SUPER_ADMIN" ? "Super Admin" : "Administrateur";
  ...
  return (
    <div className="flex h-full items-center justify-between px-6 bg-[#0a0a0a] border-b border-[#d4af37]/15">
      ...
      <div className="flex items-center gap-6 ml-6">
        ...
        <button type="button" className="relative cursor-pointer group" title="Notifications">
          ...
          {headerData?.unreadCount ? <span className="absolute -top-1.5 -right-1.5 min-w-[18px] h-[18px] px-1 bg-red-600 text-white text-[9px] font-bold rounded-full flex items-center justify-center shadow-md">{headerData.unreadCount > 99 ? "99+" : headerData.unreadCount}</span> : null}
        </button>
        ...
        {/* Language Selector */}
        <button type="button" className="flex items-center gap-2 cursor-pointer group" title="Langue actuelle : français">
        ...
        {/* Profile */}
        <div className="flex items-center gap-3 group" title={headerData?.email || "Compte administrateur"}>
          {/* Avatar circle */}
          ...
          <div className="flex flex-col gap-0.5">
            <div className="flex items-center gap-1.5">
              <span className="text-[13px] font-bold text-[#e8e1d3] leading-none group-hover:text-white transition-colors">{headerData?.name || "Administrateur"}</span>
              {headerData?.role === "SUPER_ADMIN" ? <span className="text-sm leading-none">👑</span> : null}
            </div>
            <span className="text-[10.5px] text-[#a89b82] leading-none">{roleLabel}</span>
          </div>
        </div>
      </div>
    </div>
